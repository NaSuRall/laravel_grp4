...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var hourSelect = document.getElementById('appointment_hour');
  ...
  dateInput.addEventListener('change', function () {
    ...
    var _scheduleForDay$start = scheduleForDay.start_time.split(':').map(Number),
      _scheduleForDay$start2 = _slicedToArray(_scheduleForDay$start, 1),
      startHour = _scheduleForDay$start2[0];
    var _scheduleForDay$end_t = scheduleForDay.end_time.split(':').map(Number),
      _scheduleForDay$end_t2 = _slicedToArray(_scheduleForDay$end_t, 1),
      endHour = _scheduleForDay$end_t2[0];
    ...
    var selectedDateStr = dateInput.value; // format: "YYYY-MM-DD"
    var bookedHours = appointments.filter(function (appt) {
      return appt.date === selectedDateStr;
    }).map(function (appt) {
      return parseInt(appt.hour.split(':')[0], 10);
    });
